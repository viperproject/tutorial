...
Permission *amounts* in function preconditions are interpreted differently from those in method preconditions: since functions cannot modify the heap, all that matters is whether a function may *read* a location, i.e., whether it has a positive permission amount. Concrete permission amounts in function preconditions are therefore ignored (and Viper reports a warning if one is written): `acc(x.f, 1/2)`, `acc(x.f, wildcard)` and `acc(x.f)` all simply require *some* positive permission to `x.f`. Two consequences are worth pointing out. First, non-aliasing cannot be concluded from permission amounts in a function precondition; e.g., unlike in a method precondition, `acc(x.f) && acc(y.f)` does *not* imply `x != y` inside the function. Second, inside a function, a predicate instance may be unfolded (via an [`unfolding` expression](./expressions-multi.md)) even if the precondition only provides a fraction of it. The previous behaviour, in which concrete permission amounts in function preconditions were taken into account, can be restored with the command-line option `--respectFunctionPrePermAmounts`.
