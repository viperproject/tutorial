...
Note that permission amounts are interpreted differently in the preconditions of Viper *functions* (introduced [later in this tutorial](./functions.md)): since functions cannot modify the heap, only the distinction between zero and positive permission amounts matters there, and concrete permission amounts are ignored; see the [section on functions](./functions.md) for details.
