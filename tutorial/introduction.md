...
The following simple example shows a method that computes the sum of the first `n` positive natural numbers. The method declaration includes a precondition (the assertion following `requires`) to restrict the argument to non-negative values and a postcondition (the assertion following `ensures`) to guarantee properties of the result variable `res`. Method preconditions and postconditions together make up a method's *specification*.
